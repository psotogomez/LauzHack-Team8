...
from keys import TELEGRAM_KEY, INIT_PROMPT
...
async def photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    caption = img2text(tmp_photo, INIT_PROMPT)
    #await update.message.reply_text(caption)

    #caption = caption[:50]

    file_name = 'temp_audio.ogg'
    text2speech(caption, save=True, save_path='./', file_name=file_name)

    # signal, sampling_rate = audiofile.read(file_name, always_2d=True)
    # duration = signal.shape[1] / sampling_rate

    # # respond audio
    await update.message.reply_voice(file_name, caption=f"Audio description of the image: {caption}")
    #await update.message.reply_voice(file_name, caption=f"Voice note duration: {duration} seconds")
...
async def voice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    caption = speech2text(tmp_file)
    file_name_img = 'tmp_photo.jpg'
    output = img2text(file_name_img, f"Based on the image provided answer or complete the next sentence: {caption}")
    await update.message.reply_text(output)

    file_name = 'temp_audio.ogg'
    text2speech(output, save=True, save_path='./', file_name=file_name)

    await update.message.reply_voice(file_name, caption=f"Audio answering the following question: {caption}")
